...
const createSupplierValidation = [
  ...
  body('phone')
    ...
    .matches(/^09[0-9]{8}$/).withMessage('Invalid Ethiopian phone number'),
  body('email')
...
const receiveItemsValidation = [
  ...
  body('items')
    .isArray({ min: 1 }).withMessage('At least one item is required'),
  body('items.*.poItemId')
    .isInt().withMessage('PO item ID is required'),
  body('items.*.quantityReceived')
    .isInt({ min: 0 }).withMessage('Quantity received must be non-negative'),
  body('items.*.quantityDamaged')
    .isInt({ min: 0 }).withMessage('Quantity damaged must be non-negative'),
  body('items.*.qualityPass')
    .isBoolean().withMessage('Quality pass must be true or false'),
  body('receivingNote')
    .optional()
    .isString()
    .isLength({ max: 500 })
...
const poIdParamValidation = [
  param('id')
    .isInt().withMessage('Purchase order ID must be a valid integer')
    .toInt()
];
